...
async function runValidation(query, grounded, model, customPrompt) {
  ...
  for (const citation of citations) {
    ...
    if (!foundInSources && citation.type === "bva") {
      ...
      if (numMatch) {
        for (const known of knownIds) {
          if (known.includes(numMatch[1])) { foundInSources = true; break; }
        }
      }
    }

    result.status = foundInSources ? "VERIFIED" : "NOT_IN_SOURCES";
    result.detail = foundInSources
      ? "Citation found in sentinel-tagged source context"
      : "Citation NOT found in any [SOURCE_START]...[SOURCE_END] block";
  ...
  steps.push({
    step: "summary",
    data: { total: results.length, verified, ungrounded, notInSources, hallucinated, liveApi: !!BVA_API },
  });

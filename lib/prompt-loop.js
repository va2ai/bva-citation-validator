...
export async function runPromptLoop(options) {
  ...
  for (let i = startIteration; i < startIteration + maxIterations; i++) {
    ...
    // 5. Save state after every iteration (crash recovery)
    await saveState({ bestPrompt, bestScore, initialScore: history[0]?.score ?? 0, history, totalIterations: history.length });
    ...
    // 7. Generate next prompt candidate

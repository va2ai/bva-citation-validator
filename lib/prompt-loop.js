...
function buildLedger(history) {
  ...
  if (kept.length > 0) {
    ...
    for (const h of kept) {
      ...
      // Show what rules were applied (stored on the previous iteration's suggestionsForNext)
      const rulesApplied = prev?.suggestionsForNext;
  ...
  if (reverted.length > 0) {
    ...
    for (const h of reverted) {
      ...
      if (rulesApplied && rulesApplied.length > 0) {
        ...
        lines.push(`    (no specific rule changes recorded)`);
      }
    }
    lines.push("");
  }

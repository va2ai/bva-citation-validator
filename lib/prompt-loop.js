...
// ---------------------------------------------------------------------------
// Persistence: save/load optimization state to disk
// ---------------------------------------------------------------------------
...
/**
 * Save optimization run results to disk.
 * - optimize/latest-prompt.txt — best prompt (ready to copy/use)
 * - optimize/history.jsonl — append-only log of all runs
 * - optimize/state.json — full state for resuming
 */
async function saveRun(result) {
  await ensureDir();

  // Best prompt as plain text (easy to read/copy)
  await writeFile(join(OPTIMIZE_DIR, "latest-prompt.txt"), result.bestPrompt, "utf-8");

  // Append run summary to history log
  const summary = {
  ...
  await writeFile(
    join(OPTIMIZE_DIR, "history.jsonl"),
...
/**
 * Load previous optimization state from disk.
 * Returns null if no previous state exists.
 */
export async function loadPreviousState() {
  try {
    const data = await readFile(join(OPTIMIZE_DIR, "state.json"), "utf-8");
...
export async function loadLatestPrompt() {
  try {
    return await readFile(join(OPTIMIZE_DIR, "latest-prompt.txt"), "utf-8");
  } catch {
    return null;
  }
}
 ...
 * @param {boolean} [options.resume=false] - Resume from previous optimization state
...
export async function runPromptLoop(options) {
  ...
  if (resume) {
    ...
    if (prev) {
      ...
      // Carry forward previous history for context
  ...
  for (let i = 0; i < maxIterations; i++) {
    ...
    if (avgScore > bestScore) {
      ...
      await saveRun({ bestPrompt, bestScore, initialScore: history[0]?.score ?? 0, history, totalIterations: history.length });
    } else {

...
function buildLedger(history) {
  ...
  // What worked: rules from kept iterations (diff from previous)
  ...
  if (kept.length > 0) {
    ...
    for (const h of kept) {
      ...
      lines.push(`  Iter ${h.iteration}: +${scoreDelta.toFixed(1)} points — prompt grew from ${(prev?.prompt || "").length} to ${h.prompt.length} chars`);
    }
    lines.push("");
  }

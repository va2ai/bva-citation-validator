...
async function runPipeline(query, systemPrompt, model, optimizerModel, apiUrl) {
  ...
  // Extraction + critic use optimizer model (defaults to Haiku)
  const { citations } = await extractCitations(genResponse.text, anthropicClient, optimizerModel);
  const results = await validateCitations(citations, apiUrl);
  const criticResult = await runCritic(contextBlock, genResponse.text, results, anthropicClient, optimizerModel);
...
export async function runPromptLoop(options) {
  ...
  for (let i = startIteration; i < startIteration + maxIterations; i++) {
    ...
    for (const query of queries) {
      const result = await runPipeline(query, currentPrompt, model, optimizerModel, apiUrl);

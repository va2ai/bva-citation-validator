 ...
 *
 * GROUNDED_PROMPT is loaded from system-prompt.txt if it exists
 * (written by the optimizer on each improvement), falling back to
...
const DEFAULT_GROUNDED_PROMPT = `You are a VA disability claims research assistant. You answer questions from VA-accredited attorneys, VSOs, and claims agents using ONLY the source materials provided.
...
// Load from file if optimizer has written an improved version
function loadGroundedPrompt() {
  try {
    const content = readFileSync(PROMPT_FILE, "utf-8").trim();
    if (content) return content;
  } catch { /* file doesn't exist yet */ }
  return DEFAULT_GROUNDED_PROMPT;
}

export const GROUNDED_PROMPT = loadGroundedPrompt();
export { DEFAULT_GROUNDED_PROMPT };

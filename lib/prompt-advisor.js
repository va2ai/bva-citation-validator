...
export async function suggestPromptUpdates(currentPrompt, criticFindings, validationResults, client, model, history) {
  ...
  if (history && history.length > 0) {
    historySummary = `\n\nOPTIMIZATION HISTORY (${history.length} previous iterations):
${history.map((h) =>
  `Iteration ${h.iteration}: score=${h.score}${h.improved ? " (IMPROVED)" : " (reverted)"}${h.convergedReason ? ` [${h.convergedReason}]` : ""}`
).join("\n")}

IMPORTANT: Do NOT suggest rules that are already in the current prompt. Focus on NEW rules that address issues the current prompt still allows. If previous iterations show the same score, try a fundamentally different approach rather than incremental tweaks.`;
  }

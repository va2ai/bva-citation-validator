 ...
 * @param {Array} [history] - Previous optimization iterations [{iteration, prompt, score, ...}]
 * @param {string} [ledger] - Running optimization summary (what worked, what failed, persistent issues)
 * @returns {Promise<{suggestions: Array, updated_prompt: string, usage: object}>}
 */
export async function suggestPromptUpdates(currentPrompt, criticFindings, validationResults, client, model, history, ledger) {
  ...
  if (history && history.length > 0) {
    // Separate improved vs reverted iterations
    const improved = history.filter((h) => h.improved);
    const reverted = history.filter((h) => !h.improved);

    historySummary = `\n\nOPTIMIZATION HISTORY (${history.length} iterations):
${history.map((h) =>
  `Iteration ${h.iteration}: score=${h.score}${h.improved ? " (KEPT)" : " (REVERTED)"}${h.convergedReason ? ` [${h.convergedReason}]` : ""}`
).join("\n")}`;

    // Include reverted prompts so the advisor learns from failures
    if (reverted.length > 0) {
      historySummary += `\n\nFAILED ATTEMPTS (${reverted.length} prompts that scored WORSE and were reverted):
${reverted.map((h) => {
  // Show a diff summary: what was different about the failed prompt
  const promptPreview = h.prompt.length > 300 ? h.prompt.slice(0, 300) + "..." : h.prompt;
  return `--- Iteration ${h.iteration} (score: ${h.score}) ---\n${promptPreview}`;
}).join("\n\n")}

LEARN FROM THESE FAILURES: The prompts above made things WORSE. Do NOT repeat their approach. Analyze what they changed and why it failed, then try a fundamentally different strategy.`;
    }

    historySummary += `\n\nIMPORTANT: Do NOT suggest rules that are already in the current prompt. If previous iterations show the same score, try a fundamentally different approach rather than incremental tweaks.`;
  }
...
VALIDATION RESULTS:
${validationSummary}${historySummary}${ledger ? `\n\n${ledger}` : ""}`,
      },
    ],
  });
